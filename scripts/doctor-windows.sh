...
echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║                                                                     ║${NC}"
echo -e "${CYAN}║   🏥 SANCTUM DOCTOR (Windows WSL2)                                 ║${NC}"
echo -e "${CYAN}║                                                                     ║${NC}"
echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
...
if grep -qi microsoft /proc/version 2>/dev/null; then
    check_pass "WSL2 detectado"
else
    check_warn "Não parece ser WSL2 (pode ser Ubuntu normal)"
fi
...
if [[ -f "$SANCTUM_DIR/install-windows.sh" ]]; then
    check_pass "install-windows.sh presente"
else
    check_warn "install-windows.sh não encontrado"
fi
...
# Terminal (tmux)
# ============================================
section "Terminal (tmux)"

if command -v tmux &> /dev/null; then
    check_pass "tmux instalado"
else
    check_fail "tmux não instalado"
fi
...
# ============================================
# Windows/Alacritty (informativo)
# ============================================
section "Windows (Alacritty)"

echo -e "  ${YELLOW}ℹ${NC}  Alacritty deve ser instalado no Windows, não no WSL2"
echo -e "  ${YELLOW}ℹ${NC}  Execute no PowerShell: winget install Alacritty.Alacritty"
echo -e "  ${YELLOW}ℹ${NC}  Config: %APPDATA%\\alacritty\\alacritty.toml"
...
if [[ $FAIL -eq 0 ]]; then
    if [[ $WARN -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}  ✨ Tudo perfeito! Seu Sanctum está pronto no WSL2.${NC}"
    else
        echo -e "${YELLOW}${BOLD}  ⚠️  Quase lá! Alguns avisos para verificar.${NC}"
    fi
else
    echo -e "${RED}${BOLD}  ❌ Alguns itens precisam de atenção.${NC}"
    echo -e "     Execute: ${CYAN}./install-windows.sh${NC} para corrigir"
fi
